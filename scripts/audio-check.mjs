...
console.log(`readWave: ${samples.length} samples @ ${wave.sampleRate}Hz  mean|amp|=${(samples.reduce((a, b) => a + Math.abs(b), 0) / samples.length).toFixed(4)}`)
...
const vad = new sherpa.Vad(
  {
    sileroVad: { model: join(M, 'silero_vad.onnx'), threshold: 0.5, minSilenceDuration: 0.25, minSpeechDuration: 0.1, windowSize: 512 },
    sampleRate: 16000,
    numThreads: 1,
    provider: 'cpu',
  },
...
for (let o = 0; o + 512 <= samples.length; o += 512) {
  ...
  while (!vad.isEmpty()) { segs++; vad.pop() }
}
