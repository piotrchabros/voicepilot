...
console.log(`samples=${wave.samples.length} sr=${wave.sampleRate} peak=${peak.toFixed(3)} mean|amp|=${(wave.samples.reduce((a, b) => a + Math.abs(b), 0) / wave.samples.length).toFixed(4)}`)
