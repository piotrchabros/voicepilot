...
| 6.9 | [lane:gate] [tdd:required] Bench/metrics + Phase 5 merge note: analysis latency p50/p95 + tokens/call を bench に追加（`BenchStage` とは分離）、card p95 が baseline 不変であることを検証。AnalysisEngine/`AnalysisMsg` が 5.1 service lift に乗る旨を docs に記録 | bench 出力に analysis p50/p95 + tokens/call、card p95 不変 evidence、merge note 存在 | 6.4, 6.5 | cc:WIP |
